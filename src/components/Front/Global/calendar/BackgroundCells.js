                ...
                return /*#__PURE__*/ _react.default.createElement(
                    ...
                    range.map(function (date, index) {
                        ...
                        if (selected) {
                            let find = null;
                            if (monthIndex) {
                                find = container.querySelector(`#rbc-row-index-${index}-${weekIdx - 1}-${monthIndex}`);
                            } else {
                                find = container.querySelector(`#rbc-row-index-${index}-${weekIdx - 1}`);
                            }
                            if (find) {
                                top = false;
                            }
                        ...
                        let uniqIndex = `rbc-row-index-${index}-${weekIdx}`;

                        if (monthIndex) {
                            uniqIndex = `rbc-row-index-${index}-${weekIdx}-${monthIndex}`;
                        }

                        const borderCss = '1px solid #777D87';
                        return (
                            <div
                                key={index}
                                id={selected ? uniqIndex : ''}
                                className={`rbc-day-bg ${className ? className : ''}
                                    ${selected ? 'rbc-selected-cell' : ''}
                                    ${localizer.isSameDate(date, current) ? 'rbc-today' : ''}`}
                                style={{
                                    ...style,
                                    borderLeft:
                                        selected && (startIdx === index || (startIdx < 0 && index === 0))
                                            ? borderCss
                                            : '',
                                    borderTop: selected && top ? borderCss : '',
                                    borderRight:
                                        selected && (endIdx === index || (endIdx > 6 && index === 6)) ? borderCss : '',
                                    borderBottom: selected && bottom ? borderCss : '',
                                }}
                            ></div>
                        );
                    })
            ...
            value: function _selectable() {
                ...
                selector.on('selecting', function (box) {
                    var _this2$props2 = _this2.props,
                        ...
                        isSelecting = _this2$props2.isSelecting ?? true,
                ...
                selector.on('beforeSelect', function (box) {
                    document.querySelectorAll('.rbc-day-bg').forEach((item) => {
